...
function clamp(value, min, max){
    ...
    function subscribeToCurrentRoom(roomId){
      ...
      currentRoomUnsubscribe = subscribeToRoom(currentRoomId, (roomData)=>{
        ...
        if(roomData && roomData.gameState) applyRoomGameState(roomData.gameState);
        renderOnlineRoomState();
      });
    }
    ...
    function openPlayingXIEditor(playerIndex){
      ...
      for(let i=0;i<11;i++){
        ...
        if(p.playing && p.playing.xi && p.playing.xi[i]) sel.value = p.playing.xi[i];
        selects.push(sel);
        row.appendChild(sel);
      ...
      for(let ov=1; ov<=20; ov++){
        ...
        if(p.playing && Array.isArray(p.playing.bowlingPlan) && p.playing.bowlingPlan[ov - 1]){
      ...
      bowlingSelects.forEach(s=> s.addEventListener("change", refreshOptions));
      // initial refresh
      refreshOptions();
    }
